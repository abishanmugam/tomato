    ...
    try {
      ...
      return serviceGetCapitalFlow(params).then((res) => {
        res.rows = res.rows.map((el: any, idx: number) => {
          ...
          el.order = idx + 1;
          el.__createdAt__ =
            moment(el.createdAt).format(FORMAT_DATE_MINUTE) + suffix;
    ...
    switch (type) {
      ...
      case FilterType.Yesterday:
        const prevDay = moment(
          moment().subtract(1, 'days').format(FORMAT_DATE),
          FORMAT_DATE
        );
        date[0] = prevDay;
        date[1] = prevDay;
      ...
      case FilterType.LastWeek:
        date[0] = moment(
          moment().subtract(7, 'days').format(FORMAT_DATE),
          FORMAT_DATE
        );
      ...
      case FilterType.NextMonth:
        ...
        date[1] = moment(
          moment(startDate).add(1, 'month').endOf('month').format(FORMAT_DATE),
          FORMAT_DATE
        );
        break;
      ...
      case FilterType.ThisYear:
        date[0] = moment(
          moment(startDate).startOf('year').format(FORMAT_DATE),
          FORMAT_DATE
        );
        date[1] = moment(
          moment(startDate).endOf('year').format(FORMAT_DATE),
          FORMAT_DATE
        );
        break;
